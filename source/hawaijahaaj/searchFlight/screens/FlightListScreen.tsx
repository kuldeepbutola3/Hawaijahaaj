...
// import { AppRoutes, ApptNavigationProp } from '../../../navigation/RootNav';
// import { doFetchRefreshToken } from '../../session/sessionSlice';
// import { fetchFlight } from '../flightSlice';
import { appColors } from '../../../styles/appColors';
import { formatDate } from '../../../utils/date-formatter';
import { Screen } from '../../../components/Screen';
import { GetFlightParam } from '../../../hw/flight/FlightSearchRequestModel';
import { SearchRouteProp } from '../searchStack/SearchStack';
import { FilterModel, TimeSplit } from '../../../hw/flight/filterModel';
import { FlightSet } from '../../../hw/flight/FlightModel';
// import { AppRouteProp } from '../../../navigation/AppNav';
// import { SearchRouteProp, SearchStackProp } from '../searchStack/SearchStack';
// import { AppRouteProp, ApptNavigationProp } from '../../../navigation/AppNav';
...
export const FlightListScreen: AuraStackScreen = () => {
  ...
  const { param } = useRoute<SearchRouteProp<'FlightList'>>().params;

  console.log(
    'flightDetail--------1',
    JSON.stringify(flightDetail?.results?.length),
  );

  const tooltipRefNonStop = useRef<typeof Tooltip>(null);
  ...
  //   ? Math.max(
  //       flightDetail?.results[0][flightDetail?.results[0].length - 1].fare
  //         .publishedFare ?? 0,
  //       flightDetail?.results[1][flightDetail?.results[1].length - 1].fare
  //         .publishedFare ?? 0,
  //     )
  //   : flightDetail?.results[0][flightDetail?.results[0].length - 1].fare
  //       .publishedFare ?? 0;
  ...
  //   }
  // }, [minRange, maxPrice, minPrice]);

  /**  filter states */
  const [priceFilte, setPriceFilte] = useState(false);
  const [stops, setStops] = useState(-1);
  const [flightArray, setFlightArray] = useState([] as Array<string>);
  const [timeSelected1, setTimeSelected1] = useState(
    undefined as TimeSplit | undefined,
  );
  const [timeSelected2, setTimeSelected2] = useState(
    undefined as TimeSplit | undefined,
  );

  // /** getting filter array for response  */
  // const uniqueFlightSet = flightDetail?.results;
        ...
        <View style={styles.filterSegment}>
          <View style={styles.filterSegmentInnerView}>
            <Button
              ...
              titleStyle={
                stopsInner === 0 ? styles.titleSelected : styles.titleUnSelected
              }
              title={`0 ${t('nonStop')}`}
              onPress={onPress(0)}
            />
          ...
          <View style={styles.filterSegmentInnerView}>
            <Button
              ...
              titleStyle={
                stopsInner === 1 ? styles.titleSelected : styles.titleUnSelected
              }
              type="clear"
              title={`1 ${t('stop')}`}
              onPress={onPress(1)}
        ...
        <View style={styles.filterSegment}>
          <View style={styles.filterSegmentInnerView}>
            <Button
              ...
              titleStyle={
                stopsInner === 2 ? styles.titleSelected : styles.titleUnSelected
              }
              title={`2+ ${t('stops')}`}
              onPress={onPress(2)}
            />
    ...
    return (
      <View style={style}>
        <FlightDepartureFilter
          ...
          city={
            flightDetail?.results[0][0].segments[0][0].origin?.airport
              ?.cityName ?? ''
          }
        ...
        {flightDetail?.results.length > 1 && (
          <FlightDepartureFilter
            ...
            // viewStyle={styles.timeListInner}
            city={
              flightDetail?.results[1][0].segments[0][0].origin?.airport
                ?.cityName ?? ''
            }
            // close={tooltipRefTime.current?.toggleTooltip}
            timeSelected={seT2}
  ...
  // const resultsArray = [];
  const resultsArray = flightDetail?.results ?? [];
  ...
  const getTimeFilterData = (itm: Array<Array<FlightSet>>) => {
    ...
    if (timeSelected1 && itm.length) {
      ...
      array.push(
        array1.filter(i => {
          const time = Number(
            formatDate(i.segments[0][0].origin.depTime, 'HH'),
          );
      ...
      itm.map(data => {
        flghtSearch.push(
          data.filter(
            i =>
              flightArray.indexOf(
                i.segments[0][0].airline?.airlineName ?? '',
              ) >= 0,
          ),
        );
      });
  ...
  if (flightDetail?.results && Array.isArray(flightDetail?.results)) {
    ...
    // const stopFilter =
    //   stops === -1 ? [...flightDetail?.results] : getFilterData();
  ...
  return (
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      {loading === 'pending' ? (
        ...
        <>
          <Text>resultsArray. -------{resultsArray.length}</Text>
          <View style={styles.container}>
            {resultsArray.map((i, index) => (
              <FlightList
                key={`FlightKey${RandomNumber}--${index}`}
                items={i}
                show={boolValue}
              />
            ))}
