...
import { FlightClass } from '../flightSearchModel';
...
export const FlightClassCard: FC<{
  initialClass: FlightClass;
  onPressDone: (sender: FlightClass) => void;
}> = ({ initialClass, onPressDone }) => {
  const [selectedClass, setSelectedClass] = useState<FlightClass>(initialClass);

  const onPress = (flightClass: FlightClass) => {
    return () => {
