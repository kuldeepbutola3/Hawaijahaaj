...
export const FlightFilterScreen: AuraStackScreen = () => {
  ...
  const [timeSelected1, setTimeSelected1] = useState(
    param.timeSplit1 as TimeSplit | undefined,
  );
  const [timeSelected2, setTimeSelected2] = useState(
    param.timeSplit2 as TimeSplit | undefined,
  );
  ...
  const currency =
    flightDetail?.results[0][0]?.fare?.currency === 'INR' ? '₹' : '';

  const [maxRang, setMaxRange] = useState(
    param.priceRange?.higher ?? maxPrice ?? 0,
  );
  const [minRange, setMinRange] = useState(
    param.priceRange?.lower ?? minPrice ?? 0,
  );

  const [flightArray, setFlightArray] = useState(
    param.flightArray ?? ([] as Array<string>),
  );
          ...
          <View style={styles.filterSegment}>
            <View style={styles.filterSegmentInnerView}>
              <Button
                ...
                titleStyle={
                  stops === 0 ? styles.titleSelected : styles.titleUnSelected
                }
                title={`0 ${t('nonStop')}`}
            ...
            <View style={styles.filterSegmentInnerView}>
              <Button
                ...
                titleStyle={
                  stops === 1 ? styles.titleSelected : styles.titleUnSelected
                }
                type="clear"
                title={`1 ${t('stop')}`}
            ...
            <View style={styles.filterSegmentInnerView}>
              <Button
                ...
                titleStyle={
                  stops === 2 ? styles.titleSelected : styles.titleUnSelected
                }
                type="clear"
                title={`2 ${t('stop')}`}
