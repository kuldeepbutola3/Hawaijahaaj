...
export const SSRScreen: AuraStackScreen = () => {
  ...
  const allTravellers: Array<Traveller> = [
    ...travellerAdult,
    ...travellerChild,
  ];
  const initialFlightState: SelectionState = {
    seats: {},
    meals: {},
    baggage: {},
  ...
  const [flightState, dispatchToFlightReducer] = useReducer(
    flightReducer,
    initialFlightState,
  );
  ...
  const getRoutes = () => {
    const routes =
      flightFare?.artSSRResponse?.ssrInnerResponse?.seatDynamic.map(
    ...
    switch (route.key) {
      case 'seats':
        return (
          <SeatsTab
            seats={
              flightFare?.artSSRResponse?.ssrInnerResponse?.seatDynamic?.[0]
                ?.segmentSeat?.[0]?.rowSeats || []
            }
            dispatchToFlightReducer={dispatchToFlightReducer}
            selectedSeat={
              flightState?.seats?.[flightState?.source]?.[
                flightState?.traveller
              ] || {}
            }
      ...
      case 'meals':
        return (
          <MealsTab
            meals={
              flightFare?.artSSRResponse?.ssrInnerResponse?.mealDynamic?.[0] ||
              []
            }
            dispatchToFlightReducer={dispatchToFlightReducer}
            selectedMeals={
              flightState?.meals?.[flightState?.source]?.[
                flightState?.traveller
              ] || []
            }
      ...
      case 'baggage':
        return (
          <BaggageTab
            baggage={
              flightFare?.artSSRResponse?.ssrInnerResponse?.baggage?.[0] || []
            }
            dispatchToFlightReducer={dispatchToFlightReducer}
            selectedBaggage={
              flightState?.baggage?.[flightState?.source]?.[
                flightState?.traveller
              ] || []
            }
    ...
    return (
      <View>
        <TabBar
          ...
          renderLabel={({ route }) => (
            <Text style={{ color: appColors.black, margin: 8, fontSize: 18 }}>
              {route.title}
            </Text>
          )}
        />
  ...
  }, [navigation, flightState, totalPrice, param]);
        ...
        <BottomSheet
          ...
          <ReviewBooking
            ...
            seatCount={`${selectedSeats?.length}/${
              allTravellers?.length * getRoutes()?.length || 0
            }`}
            selectedBaggages={totalMealsPrice.meals}
            baggageCount={''}
            selectedMeals={''}
            mealCount={''}
